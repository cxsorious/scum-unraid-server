...
for dir in "$SERVER_DIR" "$BACKUP_DIR" "/serverdata/steamcmd" "$WINEPREFIX"; do
    echo "[INFO] สร้างและตั้งสิทธิ์ $dir"
    mkdir -p "$dir"
    chown -R ${PUID}:${PGID} "$dir" 2>/dev/null || echo "[WARN] ไม่สามารถ chown $dir (อาจเป็น mount จาก host)"
done
...
if [ ! -f "$SERVER_BIN" ]; then
    echo "[ERROR] ไม่พบ SCUMServer.exe ใน $SERVER_BIN"
    exit 1
fi
